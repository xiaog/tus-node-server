...
class FileUpyunStore extends DataStore {
    ...
    create(req) {
        return new Promise((resolve, reject) => {
            ...
            const metadata = upload_metadata.split(',');
            const file_type = path.extname(new Buffer(metadata[2].split(' ')[1], 'base64').toString('ascii'));
            ...
            return fs.open(`${this.directory}/${file.id}`, 'w', (err, fd) => {
                ...
                return fs.close(fd, (exception) => {
                    ...
                    return this.upyun.putFile(file.extensions.remote_dir, '', initOptions, (error, res) => {
                        if (error) {
                            return reject(error);
                        }
                        this.next_id = 0;
                        ...
                        if (res.headers['x-upyun-multi-uuid']) {
                            this.configstore.set(`upyun-uuid:${file.id}`, `${res.headers['x-upyun-multi-uuid']},0,${res.headers['x-upyun-next-part-size']}`);
                        }
                        // if (res.headers['x-upyun-next-part-size']) {
                        //     this.upyunNextPart = res.headers['x-upyun-next-part-size'];
                        // }

                        this.emit(EVENTS.EVENT_FILE_CREATED, { file }, resolve, reject);
                        return resolve(file);
                    });
    ...
    write(req, file_id, offset) {
        return new Promise((resolve, reject) => {
            ...
            req.on('end', () => {
                ...
                const upyunStats = this.configstore.get(`upyun-uuid:${file_id}`).split(',');
                console.log(upyunStats);
                const upyun_uuid = upyunStats[0];
                const upyun_next_id = upyunStats[1];
                const upyun_next_part = upyunStats[2];
                
                const opts = {
                    'X-Upyun-Multi-Stage': 'upload',
                    'X-Upyun-Part-ID': String(upyun_next_id),
                    'X-Upyun-Multi-Length': upyun_next_part,
                };
                if (upyun_uuid) {
                    opts['X-Upyun-Multi-UUID'] = upyun_uuid;
                }
                ...
                return this.upyun.putFile(config.extensions.remote_dir, block, opts, (error, res) => {
                    if (error) {
                        return reject(error);
                    }
                    if (Number(res.statusCode) !== 204) {
                    ...
                    this.configstore.set(`upyun-uuid:${file_id}`, `${res.headers['x-upyun-multi-uuid']},${res.headers['x-upyun-next-part-id']},${res.headers['x-upyun-next-part-size']}`);
                    if (Number(res.headers['x-upyun-next-part-id']) !== -1) {
                        // this.next_id = res.headers['x-upyun-next-part-id'];
                        return resolve(offset);
                    }

                    this.upyun.putFile(config.extensions.remote_dir, '', {
                        'X-Upyun-Multi-Stage': 'complete',
                        'X-Upyun-Multi-UUID': upyun_uuid
                    }, (err, response) => {
                        if (err) {
                            return reject(err);
                        }
                        ...
                        ffmpeg.ffprobe(file_path, (error, metadata) => {
                            ...
                            if (metadata) {
                                ...
                                if ((videoDuration / 60) > 15) {
                                    this.emit(ERRORS.EVENT_DELETE_UPYUN_FILE, config);
                                    return reject(ERRORS.VIDEO_DURATION_ERROR);
                                }
                            }
                            this.emit(EVENTS.EVENT_UPLOAD_COMPLETE, config);
                            return resolve(offset);
                        });
                    });
                });
