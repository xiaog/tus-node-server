            ...
            return fs.open(`${this.directory}/${file.id}`, 'w', (err, fd) => {
                ...
                return fs.close(fd, (exception) => {
                    ...
                    const initOptions = {
                        ...
                        'X-Upyun-Multi-Type': upload_metadata,
                        'X-Upyun-Multi-Length': file.upload_length
                    };
                    this.upyun.putFile(file.id + this.file_type, `${this.directory}/${file.id}`, null, true, initOptions, (err, res) => {
                        if (err) return reject(err);
                        if (res.headers.statusCode != 204) return reject(ERRORS.UPYUN_WRITE_ERROR);

                        this.headers = res.headers;
                        if (res.headers['x-upyun-multi-uuid']) this.upyunUUID = res.headers['x-upyun-multi-uuid'];
                        if (res.headers['x-upyun-next-part-size']) this.upyunNextPart = res.headers['x-upyun-next-part-size'];
    ...
    write(req, file_id, offset) {
        return new Promise((resolve, reject) => {
            ...
            req.on('end', () => {
                ...
                let totalLength = chunk.map((item) => item.length ).reduce((acc, val) => {return acc+val}, 0);
                let block = Buffer.concat(chunk, totalLength);
                
                this.upyun.putFile(file_id + this.file_type, block, null, true, opts, (error, res) => {
                   ...
                   if (res.headers.statusCode != 204) return reject(ERRORS.UPYUN_WRITE_ERROR);
                    ...
                    opts['X-Upyun-Multi-Stage'] = 'complete';
                    this.upyun.putFile(file_id + this.file_type, block, null, true, opts, (err, res) => {
                        if (err) return reject(err);
                        if (res.headers.statusCode != 204 || res.headers.statusCode != 201) return reject(ERRORS.UPYUN_WRITE_ERROR);
                        resolve(offset);
                    });
                });
            });
