        ...
        return new Promise((resolve, reject) => {
            ...
            return fs.open(`${this.directory}/${file.id}`, 'w', (err, fd) => {
                ...
                return fs.close(fd, (exception) => {
                    ...
                    return this.upyun.putFile(file.id + this.file_type, `${this.directory}/${file.id}`, null, true, initOptions, (error, res) => {
                        if (error) {
                            return reject(error);
                        }
    ...
    write(req, file_id, offset) {
        return new Promise((resolve, reject) => {
            ...
            req.on('end', () => {
                ...
                return this.upyun.putFile(file_id + this.file_type, block, null, true, opts, (error, res) => {
                    if (error) {
                        return reject(error);
                    }
                    ...
                    return this.upyun.putFile(file_id + this.file_type, block, null, true, opts, (err, response) => {
                        if (err) {
                            return reject(err);
                        }
