...
class FileUpyunStore extends DataStore {
    ...
    create(req) {
        return new Promise((resolve, reject) => {
            ...
            const remote_dir = req.remote_dir;
            ...
            const file = new File(file_id, upload_length, upload_defer_length, upload_metadata, remote_dir);
            ...
            return fs.open(`${this.directory}/${file.id}`, 'w', (err, fd) => {
                ...
                return fs.close(fd, (exception) => {
                    ...
                    return this.upyun.putFile(file.remote_dir, `${this.directory}/${file.id}`, null, true, initOptions, (error, res) => {
                        if (error) {
                            return reject(error);
                        }
    ...
    write(req, file_id, offset) {
        return new Promise((resolve, reject) => {
            ...
            req.on('end', () => {
                ...
                return this.upyun.putFile(config.remote_dir, block, null, true, opts, (error, res) => {
                    if (error) {
                        return reject(error);
                    }
                    if (Number(res.statusCode) !== 204) {
                    ...
                    return this.upyun.putFile(config.remote_dir, block, null, true, opts, (err, response) => {
                        if (err) {
                            return reject(err);
                        }
